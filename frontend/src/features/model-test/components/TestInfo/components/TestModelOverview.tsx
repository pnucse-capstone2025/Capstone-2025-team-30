import { useExperimentStore } from '@/store/experimentStore';
import { StatusBadge } from '@/shared/components';
import { formatDate } from '@/shared/utils/dateUtils';
import {
  OverviewInfoTable,
  OverviewInfoRow,
  OverviewInfoLabel,
  OverviewInfoValue
} from '@/shared/components/styles/OverviewInfo';
import type { JSX } from 'react';

/**
 * 테스트 모델 개요 컴포넌트
 * 선택된 테스트 모델의 기본 정보를 표시하는 컴포넌트
 * 모델 이름, 환경, 알고리즘, 상태, 생성일 등의 정보를 테이블 형태로 보여줌
 * 
 * @returns JSX.Element
 */
export default function TestModelOverview(): JSX.Element {
  // 전역 상태에서 테스트 모델 상세 정보 가져오기
  const testModelDetailInfo = useExperimentStore((state) => state.testModelDetailInfo);

  // 모델 정보 추출 - 기본값 '-'로 설정하여 undefined 방지
  const modelName = testModelDetailInfo?.runName ?? '-';
  const envName = testModelDetailInfo?.envName ?? '-';
  const algName = testModelDetailInfo?.algName ?? '-';
  const createdAt = testModelDetailInfo?.createdAt ?? '-';
  const status = testModelDetailInfo?.status ?? '-';

  return (
    <OverviewInfoTable>
      <OverviewInfoRow $isLast={false}>
        <OverviewInfoLabel>모델 이름</OverviewInfoLabel>
        <OverviewInfoValue title={modelName}>{modelName}</OverviewInfoValue>
      </OverviewInfoRow>
      <OverviewInfoRow $isLast={false}>
        <OverviewInfoLabel>환경</OverviewInfoLabel>
        <OverviewInfoValue title={envName}>{envName.toUpperCase()}</OverviewInfoValue>
      </OverviewInfoRow>
      <OverviewInfoRow $isLast={false}>
        <OverviewInfoLabel>알고리즘</OverviewInfoLabel>
        <OverviewInfoValue title={algName}>{algName.toUpperCase()}</OverviewInfoValue>
      </OverviewInfoRow>
      <OverviewInfoRow $isLast={false}>
        <OverviewInfoLabel>상태</OverviewInfoLabel>
        <OverviewInfoValue>
          {status !== '-' ? (
            <StatusBadge status={status} />
          ) : (
            '-'
          )}
        </OverviewInfoValue>
      </OverviewInfoRow>
      <OverviewInfoRow $isLast={true}>
        <OverviewInfoLabel>생성일</OverviewInfoLabel>
        <OverviewInfoValue>{createdAt !== '-' ? formatDate(createdAt) : '-'}</OverviewInfoValue>
      </OverviewInfoRow>
    </OverviewInfoTable>
  );
}