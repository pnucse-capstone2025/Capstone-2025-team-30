"""
Step 2: BC + Teacher Feedback

주요 특징:
- BC로 초기화된 Student 네트워크에 Teacher 피드백 적용
- 차별화된 학습률: CNN(5e-6), FC(2e-5)
- Teacher 네트워크가 코사인 거리를 통해 액션 피드백 제공
- RMSprop + CosineAnnealingLR 스케줄러 사용
- 동적 cnt 전략: 긍정(4→1), 중립(1), 부정(3→1)
- Warmup 50 에피소드로 안정적인 초기 학습 보장
"""
...
def epsilon_schedule(ep):
    if ep <= WARMUP_EPISODES:  # 1-50 ep (Teacher feedback 전)
        return 0.05  # 낮은 exploration
    else:  # 51+ ep (Teacher feedback 후)  
        return 0.08  # Teacher feedback 시기에 약간 증가

# Multi-LR용 Optimizer 생성
def create_fc_lr_optimizer(model, experiment_type):
    cnn_params = []
    fc_params = []
    
    for name, param in model.named_parameters():
        if any(conv in name for conv in ['conv1', 'conv2', 'conv3']):
...
student_net.load_state_dict(bcpt)
target_net.load_state_dict(bcpt)
target_net.eval()
...
writer      = SummaryWriter(log_dir=f"runs/2e-5_0.3_fc_lr_{EXPERIMENT_TYPE}_{int(time.time())}")
...
for ep in range(1, NUM_EPISODES+1):
    ...
    current_cnn_lr = current_lrs[0]
    current_fc_lr = current_lrs[1]
    ...
    for t in range(MAX_STEPS):
        ...
        if random.random() > eps:
            with torch.no_grad(): 
                act = student_net(state).argmax(1).view(1,1)
        else:
            act = torch.tensor([[random.randrange(env.action_space.n)]], device=device)
        steps_done += 1 
        
        # Teacher feedback (WARMUP_EPISODES 이후부터)
    ...
    phase = "Pre_Teacher_Warmup" if ep <= WARMUP_EPISODES else f"With_Teacher_{EXPERIMENT_TYPE}"
    ...
    # 출력
    fc_cnn_ratio = current_fc_lr / current_cnn_lr
    if ep > WARMUP_EPISODES and ep_distances:
        ...
        print(f"Ep {ep:4d} | Reward {ep_r:7.1f} | Loss {np.mean(ep_losses):7.4f} | {fb_summary} | [{phase}] | ε:{eps:.3f} | FC/CNN:{fc_cnn_ratio:.1f}x | LR:{current_cnn_lr:.1e}/{current_fc_lr:.1e}")
    else:
        print(f"Ep {ep:4d} | Reward {ep_r:7.1f} | Loss {np.mean(ep_losses):7.4f} | [{phase}] | ε:{eps:.3f} | FC/CNN:{fc_cnn_ratio:.1f}x | LR:{current_cnn_lr:.1e}/{current_fc_lr:.1e}")
...
final_save_dict = {
    ...
    'hyperparameters': {
        ...
        'LEARNING_STRATEGY': f'FC-LR Experiment: {EXPERIMENT_TYPE}',
    },
    'final_stats': {
...
torch.save(final_save_dict, f"fc_lr_experiment_{EXPERIMENT_TYPE}.pt")
print(f"\nResults saved to: fc_lr_experiment_{EXPERIMENT_TYPE}.pt")
print(f" Experiment: {EXPERIMENT_TYPE} completed!")
writer.close(); 
env.close()